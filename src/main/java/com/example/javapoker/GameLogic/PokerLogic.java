...
public class PokerLogic {
    ...
    public static void preFlopAndFlopHandling(Stack<Cards> deck) {
        gamePlayers();
        giveHands(deck, currentPlayers);
        checkBlinds();

        if(!user.isFolded()) getUserHand();
        TurnLogic.preFlopChoices(currentPlayers, (currentPlayers.indexOf(littleBlind) + 2) % players.size());
        flop(deck);

        if(!user.isFolded()) getUserHand();
        TurnLogic.turns(currentPlayers);
    }
    ...
    public static void theTurn(Stack<Cards> deck) {
        cardsList.add(deck.pop());
    ...
    public static void theRiver(Stack<Cards> deck) {
        cardsList.add(deck.pop());
        int last = cardsList.size() - 1;
